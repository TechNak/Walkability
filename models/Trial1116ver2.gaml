...
* Name: Trial1116ver2
* Author: 
* Description: This model is designed to see how people migrate to residential building when the rent of building is 40% of their income and amenities within walkable distance is more than they needed. 
* There are different income tiers with different proportion and according to income tiers, agents were given income and number of amenity they need. 
* 
* Tags: Tag1, Tag2, TagN
*/
...
species people skills: [moving]{
	...
	list<space> amenities <- all_places where (each.category = "cultural" or "HS" or "Night" or "Park" or "Restaurant" or "Shopping");
	list<space> near_amenities <- amenities within walking_distance;
	bool is_settle;
	...
	//Action to compare the option with given income and number of amenities they need. 
	action compare when: (location=start_location.location) {
		res_space_option <- one_of(all_res_places);
		if ((rent <= income*0.4) and (near_amenities>=number_of_amenity_needed){
